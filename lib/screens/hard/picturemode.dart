...
class PictureHardMode extends StatefulWidget {
  final int pictureIndex1;

  PictureHardMode(this.pictureIndex1);

  @override
  _PictureHardModeState createState() => _PictureHardModeState();
}

class _PictureHardModeState extends State<PictureHardMode> {
  ...
  void initPuzzle() {
    puzzle = List.generate(25, (index) => index);
    // Predefined number of random moves
    for (int i = 0; i < 40; i++) {
      int emptyIndex = puzzle.indexOf(24);
      List<int> possibleMoves = [];

      if (emptyIndex % 5 != 0) possibleMoves.add(emptyIndex - 1); // Left
      if ((emptyIndex + 1) % 5 != 0) possibleMoves.add(emptyIndex + 1); // Right
      if (emptyIndex - 5 >= 0) possibleMoves.add(emptyIndex - 5); // Up
      if (emptyIndex + 5 < 25) possibleMoves.add(emptyIndex + 5); // Down
    ...
    moves = 0;
    isCompleted = false;
    tileInPosition = List.filled(25, false);
    updateTileInPosition();
  }

  Future<void> completePuzzle() async {
    sharedPreferences = await SharedPreferences.getInstance();
    var bestScoreKey = '_hardpic${widget.pictureIndex1}BestScore';
    if (!sharedPreferences.containsKey(bestScoreKey) ||
        moves < sharedPreferences.getInt(bestScoreKey)!) {
      await sharedPreferences.setInt(bestScoreKey, moves);
    }
  }
        ...
        children: [
          RepaintBoundary(
            ...
            child: ImageFiltered(
              ...
              child: Image.asset(
                'assets/hard/${widget.pictureIndex1}/original.jpg',
                        ...
                        return GestureDetector(
                          ...
                          child: ClipRRect(
                            ...
                            child: Container(
                              ...
                              child: puzzle[index] == 24
                                  ? Container()
                                  : Image.asset(
                                      'assets/hard/${widget.pictureIndex1}/${puzzle[index] + 1}.jpg',
                                      fit: BoxFit.cover,
                                    ),
                            ),
                          ),
