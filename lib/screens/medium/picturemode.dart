...
class PictureMediumMode extends StatefulWidget {
  final int pictureIndex2;

  PictureMediumMode(this.pictureIndex2);

  @override
  _PictureMediumModeState createState() => _PictureMediumModeState();
}

class _PictureMediumModeState extends State<PictureMediumMode> {
  ...
  void initPuzzle() {
    puzzle = List.generate(16, (index) => index);
    // Perform a predefined number of random moves
    for (int i = 0; i < 35; i++) {
      int emptyIndex = puzzle.indexOf(15);
      List<int> possibleMoves = [];
      if (emptyIndex % 4 != 0) possibleMoves.add(emptyIndex - 1); // Left
      if ((emptyIndex + 1) % 4 != 0) possibleMoves.add(emptyIndex + 1); // Right
      if (emptyIndex - 4 >= 0) possibleMoves.add(emptyIndex - 4); // Up
      if (emptyIndex + 4 < 16) possibleMoves.add(emptyIndex + 4); // Down
      int randomMove = possibleMoves[Random().nextInt(possibleMoves.length)];
      swapTiles(emptyIndex, randomMove);
    }
    moves = 0;
    isCompleted = false;
    tileInPosition = List.filled(16, false); // Initialized all tiles to false
    updateTileInPosition();
  }

  Future<void> completePuzzle() async {
    sharedPreferences = await SharedPreferences.getInstance();
    var bestScoreKey2 = '_mediumpic${widget.pictureIndex2}BestScore';
        ...
        children: [
          RepaintBoundary(
            ...
            child: ImageFiltered(
              ...
              child: Image.asset(
                'assets/medium/${widget.pictureIndex2}/original.jpg',
                        ...
                        return GestureDetector(
                          ...
                          child: ClipRRect(
                            ...
                            child: Container(
                              ...
                              child: puzzle[index] == 15
                                  ? Container()
                                  : Image.asset(
                                      'assets/medium/${widget.pictureIndex2}/${puzzle[index] + 1}.jpg',
                                      fit: BoxFit.cover,
                                    ),
                            ),
                          ),
